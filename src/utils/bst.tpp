#ifndef STATETABLE_BST_T_H_
#define STATETABLE_BST_T_H_
...
template <typename K, typename T> bool BST<K,T>::remove(K key) {
  return (remove(key, this->root) != nullptr);
}
...
template <typename K, typename T> T* BST<K,T>::operator[](K key) {
  BSTNode<K,T>* temp = find(key, this->root);
